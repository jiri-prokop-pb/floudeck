...
- **Block types**: `block_type` column: `'scheduled'` (default, auto-runs) or `'action'` (manual-only, pill buttons). Scheduler WHERE clauses filter by `block_type = 'scheduled'`.
- **Action block prompt composition**: `composeActionBlockPrompt` replaces `{{input}}` in the action's prompt template. Different from `composeActionPrompt` which uses block output as context.
- **Action block schema**: Migration v2 adds `block_type`, `title`, `actions` columns. `actions` is JSON array of `ActionDefinition`. `title` is nullable, action blocks only.
